...
for (i in 1:KfoldsOuter){ #CV externe : pour RDA (ou autre)
  ...
  for (j in 1:KfoldsInner) { # CV interne : pour nb.pred optimal
    ...
    fit.inner <- naive_bayes(y ~ ., data=xbest.train.inner)
    pred.inner <- predict(fit.inner, newdata=xbest.test.inner[,1:ncol(xbest.train.inner)-1])
  ...
  fit <- naive_bayes(y ~ ., data=xbest.train)
  
  pred <- predict(fit, newdata=xbest.test[,1:ncol(xbest.train)-1])
