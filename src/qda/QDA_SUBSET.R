...
for (i in 1:KfoldsOuter){ #CV externe : pour RDA (ou autre)
  ...
  for (j in 1:KfoldsInner) { # CV interne : pour nb.pred optimal
    ...
    fit.inner <- qda(y ~ ., data=xbest.train.inner)
    pred.inner <- predict(fit.inner, newdata=xbest.test.inner)$class
  ...
  fit <- qda(y ~ ., data=xbest.train)
  
  pred <- predict(fit, newdata=xbest.test)$class
